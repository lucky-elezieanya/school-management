...
export default function SubjectUpload() {
  ...
  const handleUploadSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      const url = `${BASE_URL}/academics/subjects/upload/`;
      const res = await uploadFile(file, url);
      if (res) {
        alert(`Subjects uploaded successfully,
                            ${res.created_count} subjects created,
                            ${res.skipped_count} records skipped`);
        router.push("/admin/administration/subjects/new");
      }
    } catch (error: any) {
      console.log(error);
        ...
        <div className="mt-10">
          ...
          <div className="overflow-x-auto rounded-2xl border border-emerald-100 shadow-sm">
            <table className="min-w-full text-sm text-left border-collapse">
              <thead className="bg-emerald-800 text-white">
                <tr>
                  <th className="px-4 py-3 whitespace-nowrap">subject_name</th>
                  <th className="px-4 py-3 whitespace-nowrap">subject_code</th>
                </tr>
              </thead>

              <tbody className="bg-white">
                <tr className="border-b border-gray-100 hover:bg-pink-50 transition">
                  <td className="px-4 py-3 whitespace-nowrap">ENGLISH</td>
                  <td className="px-4 py-3 whitespace-nowrap">ENG</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="mt-4 bg-pink-50 border border-pink-100 rounded-2xl p-4">
            ...
            <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
              ...
              <li>Each row represents one subject record.</li>
            </ul>
          </div>
        </div>
