...
import { handleUserDelete } from "@/app/lib/api";
...
export default function NewsTable({ news, loading, refresh }: Props) {
  ...
  async function remove(item: NewsSummary) {
    ...
    try {
      ...
      const res = await handleUserDelete("news", "admin/news", item.id, "News");
      if (res) {
  ...
  if (loading)
    return (
      <div className="rounded-2xl border bg-white p-8">Loading news...</div>
    );
    ...
    <div className="space-y-6">
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full rounded-xl border py-3 pl-11 pr-4 focus:border-emerald-500 focus:outline-none"
        />
      </div>
      {!filteredNews.length ? (
        <div className="rounded-2xl border bg-white p-16 text-center">
          <Newspaper size={52} className="mx-auto text-slate-300" />
          <h2 className="mt-4 text-xl font-semibold">No articles found</h2>
          <p className="mt-1 text-slate-500">
            Create a news article to get started.
          </p>
        </div>
      ) : (
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {filteredNews.map((item) => {
                // Check if the article status is explicitly published
                const isPublished =
                  ...
                  <tr key={item.id} className="border-t">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-2">
                        {/* 3. Button replaces Link for view action */}
                        <button
                          type="button"
                          ...
                          className="rounded-lg border p-2 text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                        ...
                        <button
                          ...
                          className="rounded-lg border p-2 text-red-600 disabled:opacity-50"
                        >
                          <Trash2 size={17} />
                        </button>
                      </div>
